...
class ErrorReport(models.Model):
    ...
    uid = models.CharField(
        max_length=uid_max_length, help_text="md5 version of username"
    )
    # md5 ex: "7defb184ceadab4e79eff323359ad373"
    host = models.CharField(
        max_length=host_max_length, help_text="md5 version of hostname"
    )
    ...
    mantidSha1 = models.CharField(
        max_length=mantidSha1_max_length, help_text="sha1 for specific mantid version"
    )
    # ex: "Fedora 20 (Heisenbug)"
    osReadable = models.CharField(
        max_length=osReadable_max_length, default="", blank=True
    )
    application = models.CharField(
        max_length=application_max_length, default="", blank=True
    )

    facility = models.CharField(max_length=facility_max_length, default="", blank=True)
    exitCode = models.CharField(
        max_length=exitCode_max_length, default="", null=True, blank=True
    )
    ...
    textBox = models.CharField(max_length=free_text_max_length, default="", null="True")
    stacktrace = models.CharField(max_length=stacktrace_max_length, default="")
    cppCompressedTraces = models.CharField(
        max_length=cppCrompressedTraces_max_length, default="", blank=True
    )
    githubIssue = models.ForeignKey(
        "GithubIssue", on_delete=models.SET_NULL, blank=True, null=True
    )
...
class UserDetails(models.Model):
    name = models.CharField(
        max_length=input_box_max_length, help_text="user provided name"
    )
    email = models.CharField(
        max_length=input_box_max_length, help_text="user provided email"
    )
    ...
    def clearOrphanedRecords():
        ...
        no_refs = UserDetails.objects.annotate(
            num_refs=models.Count("errorreport")
        ).filter(num_refs=0)
        no_refs.delete()
...
class GithubIssue(models.Model):
    repoName = models.CharField(
        max_length=repoName_max_length,
        default="",
        blank=True,
        help_text="'user/repo_name': for example 'mantidproject/mantid'",
    )
    issueNumber = models.CharField(
        max_length=issueNumber_max_length, default="", blank=True
    )
...
def notify_report_received(sender, instance, signal, *args, **kwargs):
    ...
    if instance.user is None:
        if stacktrace in TEST_VALUES and textBox in TEST_VALUES:
            return
        else:
            name = "Not provided"
            email = "Not provided"
    ...
    if email in TEST_VALUES and stacktrace in TEST_VALUES and textBox in TEST_VALUES:
        return

    issue_link = ""
    if instance.githubIssue:
        issue_link = (
            f"https://github.com/{instance.githubIssue.repoName}"
            f"/issues/{instance.githubIssue.issueNumber}"
        )

    notification_thread = threading.Thread(
        target=send_notification_to_slack,
        args=(
            name,
            email,
            instance.textBox,
