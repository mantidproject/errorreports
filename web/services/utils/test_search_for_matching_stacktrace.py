...
class MatchingStackTraceSearchTest(TestCase):
    ...
    def setUp(self):
        ...
        for trace, issue_number in self.entries:
            issue = GithubIssue.objects.create(
                repoName="my/repo", issueNumber=issue_number
            )
    ...
    def test_retrieve_issue_number_with_identical_trace(self):
        for trace, issue_number in self.entries:
            self.assertEqual(
                issue_number, _search_for_matching_stacktrace(trace).issueNumber
            )

    def test_retrieve_issue_number_with_different_path_seperators(self):
        for trace, issue_number in self.entries:
            altered_trace = (
                trace.replace("/", "\\") if "/" in trace else trace.replace("\\", "/")
            )
            self.assertEqual(
                issue_number, _search_for_matching_stacktrace(altered_trace).issueNumber
            )
